...
export function UserDetail({ user }: { user: User }) {
  ...
  function exportBillingCsv() {
    ...
    downloadCsv(`${user.name}_2026-05_請求明細.csv`, rows);
  }
    ...
    <div className="space-y-5">
      ...
      <div className="card p-5">
        ...
        <div className="mt-4 pt-4 border-t border-ink-100 flex gap-2 text-[12px]">
          <button onClick={() => setStatusOpen(true)} className="btn btn-primary">ステータス変更</button>
          <button onClick={() => setMealOpen(true)} className="btn">食事設定</button>
          <button onClick={() => setTab("billing")} className="btn">請求明細</button>
          <button onClick={() => setTaskOpen(true)} className="btn">タスク追加</button>
          <button onClick={() => setHandoverOpen(true)} className="btn">申し送り記載</button>
        </div>
      </div>
        ...
        <div className="bg-err-50 border-l-4 border-err-600 rounded-r-md">
          <div className="px-4 py-3">
            <div className="flex items-start gap-3">
              ...
              <button onClick={() => setAllergyOpen(true)} className="btn text-[11px] py-0.5">編集（要確認）</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="border-b border-ink-200 flex gap-1 text-[13px] overflow-x-auto">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            {t.id === "allergy" && hasAllergyOrRestriction && (
      ...
      {tab === "meal" && (
        <section className="space-y-4">
          ...
          <div className="text-right">
            <button onClick={() => setMealOpen(true)} className="btn btn-primary text-[12px]">食事設定を編集</button>
          </div>
        </section>
      )}
      ...
      {tab === "allergy" && (
        <section className="space-y-3">
          ...
          <div className="text-right">
            <button onClick={() => setAllergyOpen(true)} className="btn btn-primary text-[12px]">編集</button>
          </div>
        </section>
      )}
        ...
        <section>
          <div className="flex items-end justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button onClick={exportBillingCsv} className="btn text-[12px]">CSV</button>
              <button onClick={() => toast("請求書PDFの出力は Phase 2 で実装予定", "info")} className="btn text-[12px]">PDF</button>
              <button onClick={() => setConfirmBilling(true)} className="btn btn-primary text-[12px]">請求確定</button>
            </div>
          </div>
      ...
      {tab === "goods" && (
        <EmptyTab onClick={() => toast("日用品使用記録の追加（このプロトでは別画面）", "info")} cta="日用品ページで管理">
          この利用者の日用品使用履歴・請求対象品をここに表示します。
        </EmptyTab>
      )}

      {tab === "docs" && (
        <EmptyTab onClick={() => toast("書類添付のアップロードは Phase 2 で実装予定", "info")} cta="書類を追加">
          利用者ごとの保険証・受給者証・契約書類の一覧と期限を管理します。
        </EmptyTab>
      )}

      {tab === "tasks" && (
        <EmptyTab onClick={() => setTaskOpen(true)} cta="＋ タスク追加">
          この利用者に紐づくタスクを一覧表示します。
        </EmptyTab>
      )}

      {tab === "handovers" && (
        <EmptyTab onClick={() => setHandoverOpen(true)} cta="＋ 申し送り記載">
          この利用者に関する申し送りタイムラインを表示します。
        </EmptyTab>
      )}

      {tab === "history" && (
        <EmptyTab onClick={doPrint} cta="印刷">
          基本情報・ステータス・食事設定・請求等のすべての変更履歴を時系列で表示します。
        </EmptyTab>
      )}
      ...
      <Drawer
        ...
        footer={
          <>
            <button className="btn text-[12px]" onClick={() => setStatusOpen(false)}>取消</button>
            <button className="btn btn-primary text-[12px]" onClick={() => { toast(`${user.name} 様 のステータスを変更しました`, "ok"); setStatusOpen(false); }}>変更を実行</button>
          </>
        }
      >
      ...
      <Modal open={mealOpen} onClose={() => setMealOpen(false)} title="食事設定の編集" size="lg" footer={<><button className="btn text-[12px]" onClick={() => setMealOpen(false)}>取消</button><button className="btn btn-primary text-[12px]" onClick={() => { toast("食事設定を保存しました", "ok"); setMealOpen(false); }}>保存</button></>}>
        <MealForm user={user} />
      </Modal>

      <Modal open={allergyOpen} onClose={() => setAllergyOpen(false)} title={`アレルギー・禁忌の編集：${user.name} 様`} footer={<><button className="btn text-[12px]" onClick={() => setAllergyOpen(false)}>取消</button><button className="btn btn-primary text-[12px]" onClick={() => { toast("アレルギー情報を更新しました。食札・配膳・業者発注に反映されます。", "warn"); setAllergyOpen(false); }}>保存</button></>}>
        ...
        <Field label="アレルゲン（カンマ区切り）"><input className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.allergies.map((a) => a.name).join("、")} /></Field>
        <Field label="禁忌・食事制限"><textarea className="w-full px-3 py-2 border border-ink-200 rounded" rows={3} defaultValue={user.restrictions.map((r) => `${r.type}：${r.detail}`).join("\n")} /></Field>
      </Modal>

      <Modal open={taskOpen} onClose={() => setTaskOpen(false)} title={`タスク追加：${user.name} 様`} footer={<><button className="btn text-[12px]" onClick={() => setTaskOpen(false)}>取消</button><button className="btn btn-primary text-[12px]" onClick={() => { toast("タスクを追加しました", "ok"); setTaskOpen(false); }}>保存</button></>}>
        <div className="space-y-3">
          <Field label="タスク名"><input className="w-full px-3 py-2 border border-ink-200 rounded" /></Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="期限"><input type="date" className="w-full px-3 py-2 border border-ink-200 rounded num" /></Field>
            <Field label="優先度"><select className="w-full px-3 py-2 border border-ink-200 rounded"><option>高</option><option>中</option><option>低</option></select></Field>
          </div>
        </div>
      </Modal>

      <Modal open={handoverOpen} onClose={() => setHandoverOpen(false)} title={`申し送り記載：${user.name} 様`} footer={<><button className="btn text-[12px]" onClick={() => setHandoverOpen(false)}>取消</button><button className="btn btn-primary text-[12px]" onClick={() => { toast("申し送りを記載しました", "ok"); setHandoverOpen(false); }}>記載</button></>}>
        <Field label="内容"><textarea className="w-full px-3 py-2 border border-ink-200 rounded" rows={4} /></Field>
        <label className="flex items-center gap-2 mt-2 text-[12px]"><input type="checkbox" /> ★ 重要としてマーク</label>
      </Modal>

      <Modal open={confirmBilling} onClose={() => setConfirmBilling(false)} title="請求確定" footer={<><button className="btn text-[12px]" onClick={() => setConfirmBilling(false)}>取消</button><button className="btn btn-primary text-[12px]" onClick={() => { toast(`${user.name} 様 5月分の請求を確定しました`, "ok"); setConfirmBilling(false); }}>確定する</button></>}>
...
function StatusForm({ user }: { user: User }) {
  return (
    <div className="space-y-4">
      ...
      <Field label="変更先">
        <select defaultValue={user.status} className="w-full px-3 py-2 border border-ink-200 rounded">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="期間 終了"><input type="date" className="w-full px-3 py-2 border border-ink-200 rounded num" /></Field>
      </div>
      <Field label="理由"><input className="w-full px-3 py-2 border border-ink-200 rounded" placeholder="例：肺炎・◯◯病院" /></Field>
...
function MealForm({ user }: { user: User }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      <Field label="朝 パン"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.breakfastBread ? "1" : "0"}><option value="1">あり</option><option value="0">なし</option></select></Field>
      <Field label="朝 ジュース"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.breakfastJuice ? "1" : "0"}><option value="1">あり</option><option value="0">なし</option></select></Field>
      <Field label="昼 業者"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.lunchVendor}><option>なし</option><option>A社</option><option>B社</option></select></Field>
      <Field label="夕 業者"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.dinnerVendor}><option>なし</option><option>A社</option><option>B社</option></select></Field>
      <Field label="食事形態"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.form}><option>普通食</option><option>きざみ</option><option>一口大</option><option>軟菜</option><option>ミキサー</option><option>ペースト</option></select></Field>
      <Field label="飲水形態"><select className="w-full px-3 py-2 border border-ink-200 rounded" defaultValue={user.meal.fluidForm}><option>常水</option><option>とろみ薄</option><option>とろみ中</option><option>とろみ濃</option></select></Field>
    </div>
  );
}
...
function EmptyTab({ children, onClick, cta }: { children: React.ReactNode; onClick: () => void; cta: string }) {
  return (
    <div className="card px-6 py-10 text-center">
      ...
      <button onClick={onClick} className="btn btn-primary text-[12px]">{cta}</button>
    </div>
  );
}
...
function StatusBadge({ s }: { s: string }) {
  ...
  return <span className={"text-[12px] px-2.5 py-1 rounded border font-semibold " + (map[s] ?? "")}>{s}</span>;
}

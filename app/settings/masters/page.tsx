  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[12px] text-ink-500 mt-0.5">
            業務マスタの編集。変更履歴は監査ログに残ります。
          </p>
        </div>
        <button className="btn text-[12px]">CSV インポート</button>
      </div>
                ...
                {cat.items.map((it) => (
                  <tr key={it.name} className="border-b border-ink-100 last:border-b-0 hover:bg-ink-50/60">
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <button className="btn btn-arrow text-[11px] py-0.5">編集</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="text-[11px] text-ink-500 pt-2 border-t border-ink-200">
        ※ このプロトタイプではマスタ編集モーダルは未実装です。商用版では各マスタ別に専用編集画面（一括編集 / 履歴 / インポート）を提供します。
